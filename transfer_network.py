...
class TransferNetwork():
    ...
    def create_transfer_edge(self,points,clusters):
        ...
        # 初始化transfer edge矩阵
        self.arcs=[[-1 for col in range(clusters_size)] for row in range(clusters_size)]
        for i in range(clusters_size):
            cluster1=clusters[i]

            #cluster是以时间序存储在clusters中的，因此transfer edge仅需在后续的cluster中寻找
            for j in range(i+1,clusters_size):
                ...
                for point in cluster1:
                    ...
                    # 判断cluster1和cluster2是否有边相连
                    # point是以时间序存储在cluster中的，因此可保证第一个检查到的边即为有效的transfer edge
                    if self.trajectory_only_edge(p_tid,i,j) and self.trajectory_pass_cluster(p_tid,cluster2):
                        self.arcs[i][j]=p_tid
                        self.arcs[j][i]=p_tid
